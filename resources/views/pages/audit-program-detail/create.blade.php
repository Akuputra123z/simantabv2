...
<div class="mx-auto max-w-3xl px-4 pb-12">
    ...
    <form action="{{ route('audit-program-detail.store') }}" method="POST">
        ...
        {{-- Header --}}
        <div class="mb-8 px-2">
            <h2 class="text-3xl font-medium tracking-tight text-gray-900 dark:text-white">Tambah Sub-Program</h2>
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-800/50">
            ...
            <div class="p-8 space-y-8">
                ...
                <div>
                    ...
                    <input type="text" name="nama_detail_program" value="{{ old('nama_detail_program') }}" 
                    ...
                    <div>
                        ...
                        <select name="jadwal" class="w-full rounded-xl border-2 border-gray-100 bg-gray-50/50 px-4 py-3.5 text-sm focus:border-blue-500 dark:bg-gray-900" required>
                            ...
                            @foreach(['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'] as $bulan)
                                <option value="{{ $bulan }}" {{ old('jadwal') == $bulan ? 'selected' : '' }}>{{ $bulan }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="laporan_akhir" value="{{ old('laporan_akhir') }}" 
                           class="w-full rounded-xl border-2 border-gray-100 bg-gray-50/50 px-4 py-3.5 text-sm focus:border-blue-500 dark:bg-gray-900" 
                ...
                <div>
                    ...
                    <textarea name="tujuan" rows="3" class="w-full rounded-xl border-2 border-gray-100 bg-gray-50/50 px-4 py-3.5 text-sm focus:border-blue-500 dark:bg-gray-900" placeholder="Jelaskan tujuan pemeriksaan..." required>{{ old('tujuan') }}</textarea>
                </div>
            </div>

            {{-- Footer Action --}}
            <div class="bg-gray-50/80 px-8 py-6 border-t border-gray-100 flex items-center justify-end gap-4 dark:bg-gray-900/50">
                <a href="{{ route('audit-program.show', $auditProgram->id) }}" class="px-6 py-2.5 text-sm text-gray-500 hover:text-gray-800 transition font-semibold italic">Batal</a>
