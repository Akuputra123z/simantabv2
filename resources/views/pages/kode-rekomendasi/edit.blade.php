...
<script>
function kodeRekomendasiEditForm() {
    return {
        ...
        openConfirmModal(event) {
            ...
            this.formTarget = event ? event.target : document.getElementById('form-kode-rekom-edit');
        ...
        async confirmAndSave() {
            ...
            const form = this.formTarget || document.getElementById('form-kode-rekom-edit');
            ...
            try {
                ...
                if (response.ok && data.success) {
                    this.successMessage = data.message || 'Data berhasil diperbarui.';
                    this.redirectUrl = data.redirect || '{{ route("kode-rekomendasi.index") }}';
                    this.showSuccessModal = true;
                } else {
                    let errMsg = data.message || 'Gagal menyimpan perubahan.';
                    if (data.errors) {
                ...
                console.error(err);
                alert('Terjadi kesalahan sistem/koneksi saat menyimpan perubahan.');
                this.isSubmitting = false;
            }
        }
    };
...
<div class="w-full space-y-6" x-data="kodeRekomendasiEditForm()" x-cloak>
    ...
    <div x-show="showConfirmModal"
        ...
        <div x-show="showConfirmModal"
            ...
            <p class="text-sm sm:text-base text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
                Apakah Anda yakin ingin menyimpan perubahan data Kode Rekomendasi ini?
            </p>
            ...
            <div class="flex items-center gap-3 justify-center">
                ...
                <button type="button"
                        ...
                        class="w-1/2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/25 transition-all hover:from-blue-700 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer">
                    Ya, Simpan Perubahan
                </button>
            </div>
        </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow max-w-3xl dark:bg-gray-900">
        ...
        <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-white">Edit Kode Rekomendasi</h2>

        <form action="{{ route('kode-rekomendasi.update', $data->id) }}" method="POST" id="form-kode-rekom-edit" @submit="openConfirmModal($event)">
            @csrf
            @method('PUT')
