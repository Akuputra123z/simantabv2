...
@php
    ...
    // Data temuan & rekomendasi terstruktur dari LHP
    $temuansInitial = $lhp->temuans->map(function ($t) {
        return [
            'id'                      => $t->id,
            ...
            'sebab'                   => $t->sebab ?? '',
            'akibat'                  => $t->akibat ?? '',
            'nilai_kerugian_negara'   => (float)($t->nilai_kerugian_negara ?? 0),
            'nilai_kerugian_daerah'   => (float)($t->nilai_kerugian_daerah ?? 0),
            'nilai_kerugian_desa'     => (float)($t->nilai_kerugian_desa ?? 0),
            'nilai_kerugian_bos_blud' => (float)($t->nilai_kerugian_bos_blud ?? 0),
            'nilai_temuan'            => (float)($t->nilai_temuan ?? 0),
            'recommendations'         => $t->recommendations->map(function ($r) {
                return [
                    'id'                  => $r->id,
                    'kode_rekomendasi_id' => $r->kode_rekomendasi_id,
                    'jenis_rekomendasi'   => $r->jenis_rekomendasi ?? 'administrasi',
...
function lhpEditWizard() {
    return {
        ...
        async confirmAndSave() {
            ...
            const txt = document.getElementById('btn-submit-edit-text');
        ...
        init() {
            window.lhpEditWizardInstance = this;
        ...
        getKodeTemuanLabel(kodeId) {
            if (!kodeId) return '';
            const found = KODE_TEMUANS_EDIT.find(k => String(k.id) === String(kodeId));
            if (!found) return '';
        ...
        getKodeRekomLabel(kodeId) {
            const found = KODE_REKOMS_EDIT.find(r => String(r.id) === String(kodeId));
            return found ? `[${found.kode}] — ${found.deskripsi}` : '';
        },

        getFilteredKodeRekoms(kodeTemuanId) {
            if (!kodeTemuanId) return KODE_REKOMS_EDIT;
            const temuanKode = KODE_TEMUANS_EDIT.find(k => String(k.id) === String(kodeTemuanId));
            if (temuanKode && Array.isArray(temuanKode.alternatif_rekom) && temuanKode.alternatif_rekom.length > 0) {
                ...
                const filtered = KODE_REKOMS_EDIT.filter(r => 
                    altList.includes(String(r.id)) || 
                    altList.includes(String(r.kode_numerik))
                );
                return filtered.length > 0 ? filtered : KODE_REKOMS_EDIT;
            }
            return KODE_REKOMS_EDIT;
        },
...
<div class="w-full space-y-6" x-data="lhpEditWizard()" x-cloak>
    ...
    <div x-show="showConfirmModal"
        ...
        <div x-show="showConfirmModal"
            ...
            <p class="text-sm sm:text-base text-gray-600 dark:text-gray-300 mb-6 leading-relaxed">
                Apakah Anda yakin ingin menyimpan perubahan data LHP ini? Pastikan perubahan data sudah sesuai.
            </p>
            ...
            <div class="flex items-center gap-3 justify-center">
                ...
                <button type="button"
                        ...
                        class="w-1/2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/25 transition-all hover:from-blue-700 hover:to-indigo-700 hover:shadow-blue-500/35 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer">
                    Ya, Simpan Perubahan
                </button>
            </div>
        </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <nav class="mb-1 flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400">
                ...
                <span>/</span>
                <a href="{{ route('lhps.show', $lhp) }}" class="hover:text-blue-600 truncate max-w-[150px] inline-block">{{ $lhp->nomor_lhp }}</a>
                <span>/</span>
                <span class="text-gray-700 dark:text-gray-300">Edit LHP</span>
            </nav>
            <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Perbarui LHP &amp; Rekomendasi</h1>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Edit data administrasi, temuan, dan rekomendasi hasil audit.</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <nav class="space-y-2.5">
                    ...
                    <button type="button" @click="setStep(1)"
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-0.5 truncate text-[11px] opacity-75">Nomor &amp; Lampiran</p>
                        </div>
                    </button>
                    ...
                    <button type="button" @click="setStep(4)" :disabled="!canGoToStep(4)"
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="text-xs font-bold">4. Finalisasi Edit</p>
                            <p class="mt-0.5 truncate text-[11px] opacity-75">Review &amp; Simpan</p>
                        </div>
                    </button>
                </nav>
            </div>
        ...
        <div class="lg:col-span-8 xl:col-span-9 min-w-0 space-y-6">
            ...
            <form action="{{ route('lhps.update', $lhp) }}" method="POST" enctype="multipart/form-data" id="form-lhp-edit" novalidate @submit="openConfirmModal($event)">
                @csrf
                @method('PUT')
            ...
            <div class="mb-6 rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-800">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" name="nomor_lhp" x-model="nomorLhp" required
                                   class="w-full rounded-xl border border-gray-300 bg-white px-3.5 py-2.5 text-sm text-gray-900 focus:border-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_lhp" x-model="tanggalLhp" onclick="this.showPicker()" required
                                   class="w-full rounded-xl border border-gray-300 bg-white px-3.5 py-2.5 text-sm text-gray-900 focus:border-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="catatan_umum" x-model="catatanUmum" rows="3"
                                      class="w-full resize-none rounded-xl border border-gray-300 bg-white px-3.5 py-2.5 text-sm text-gray-900 focus:border-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white"></textarea>
                        </div>
                    </div>

                    {{-- Dynamic Berkas Lampiran LHP Card (TailAdmin Style) --}}
                    <div class="pt-6 border-t border-gray-100 dark:border-gray-800 space-y-4">
                        ...
                        <div class="space-y-3">
                            {{-- List File Tersimpan --}}
                            @if($lhp->attachments->isNotEmpty())
                            <div class="mb-4 space-y-2">
                                <p class="text-[10px] font-bold uppercase tracking-wider text-gray-400">File Lampiran Tersimpan ({{ $lhp->attachments->count() }}):</p>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    @foreach($lhp->attachments as $att)
                                    <div class="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50/80 p-3 text-xs dark:border-gray-700 dark:bg-gray-800/60">
                                        <div class="flex items-start gap-2.5 min-w-0 flex-1 pr-2">
                                            <svg class="h-4 w-4 shrink-0 text-blue-500 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg>
                                            <div class="min-w-0">
                                                <p class="font-bold text-gray-800 dark:text-gray-200 truncate">{{ $att->file_name ?: 'Lampiran LHP' }}</p>
                                                @if($att->keterangan)
                                                    <p class="text-[11px] text-gray-500 dark:text-gray-400 line-clamp-1 mt-0.5">{{ $att->keterangan }}</p>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-2 shrink-0">
                                            <a href="{{ route('attachments.show', $att->id) }}" target="_blank" class="font-semibold text-blue-600 hover:underline dark:text-blue-400">Lihat</a>
                                            <form action="{{ route('attachments.destroy', $att->id) }}" method="POST" onsubmit="return confirm('Hapus lampiran ini?')" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="font-semibold text-red-500 hover:text-red-700">Hapus</button>
                                            </form>
                                        </div>
                                    </div>
                                    @endforeach
                            ...
                            <template x-for="(att, aIdx) in attachments" :key="att.id">
                                <div class="rounded-xl border border-gray-200 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-gray-900/50 space-y-3">
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs font-bold text-blue-600 dark:text-blue-400" x-text="`Lampiran Baru #${aIdx + 1}`"></span>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" :name="`attachments[${aIdx}][file_name]`" x-model="att.file_name" placeholder="cth: LHP Audit Tambahan (PDF)..."
                                        ...
                                        <div>
                                            ...
                                            <input type="text" :name="`attachments[${aIdx}][keterangan]`" x-model="att.keterangan" placeholder="cth: Dokumen audit tambahan..."
                                                   class="h-11 w-full rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-xs text-gray-900 focus:border-blue-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white dark:placeholder:text-gray-400">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div x-show="attachments.length === 0 && {{ $lhp->attachments->isEmpty() ? 'true' : 'false' }}" class="rounded-xl border border-dashed border-gray-200 py-6 text-center text-xs text-gray-400 dark:border-gray-800 dark:text-gray-500">
                                Belum ada berkas lampiran ditambahkan. Klik <strong>"Tambah File Baru"</strong> di atas.
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end border-t border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-900/30">
                    <button type="button" @click="nextStep()"
                        ...
                        Lanjut ke Langkah 2: Edit Temuan →
                    </button>
                </div>
            </div>
        </div>

        {{-- ── STEP 2: TEMUAN PEMERIKSAAN ── --}}
            ...
            <div class="mb-6 rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-800">
                <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-700">
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Edit data kondisi, sebab, akibat, dan rincian kerugian temuan.</p>
                    </div>
                ...
                <div class="divide-y divide-gray-100 dark:divide-gray-800">
                    <template x-for="(t, tIdx) in temuans" :key="t.id || t.key">
                        <div class="p-6 space-y-4">
                            <input type="hidden" :name="`temuans[${tIdx}][id]`" :value="t.id">
                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <div class="md:col-span-2 relative" x-data="{ search: '', open: false }">
                                    ...
                                    <div x-show="open" @click.outside="open = false" x-cloak
                                        ...
                                        <div class="max-h-60 overflow-y-auto space-y-1 pr-1">
                                            <template x-for="k in KODE_TEMUANS_EDIT.filter(item => {
                                            ...
                                            {{-- Empty Search Result --}}
                                            <div x-show="KODE_TEMUANS_EDIT.filter(item => `[${item.kode}] ${item.deskripsi || item.nama || ''}`.toLowerCase().includes(search.toLowerCase().trim())).length === 0"
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea :name="`temuans[${tIdx}][kondisi]`" x-model="t.kondisi" rows="3"
                                ...
                                <div>
                                    ...
                                    <textarea :name="`temuans[${tIdx}][sebab]`" x-model="t.sebab" rows="2"
                                ...
                                <div>
                                    ...
                                    <textarea :name="`temuans[${tIdx}][akibat]`" x-model="t.akibat" rows="2"
                                              class="w-full resize-none rounded-xl border border-gray-300 bg-white p-2.5 text-xs text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-white outline-none"></textarea>
                                </div>

                                {{-- Financial Breakdown (Sub-Grid 4-Kolom) --}}
                                <div class="md:col-span-2 pt-3 border-t border-gray-100 dark:border-gray-800">
                                    <label class="mb-2 block text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-300">
                                        Rincian Nilai Kerugian Keuangan (Opsional)
                                    </label>
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                        <div>
                                            <label class="mb-1 block text-[10px] font-bold uppercase text-gray-500">Kerugian Negara (Rp)</label>
                                            <input type="number" :name="`temuans[${tIdx}][nilai_kerugian_negara]`" x-model.number="t.nilai_kerugian_negara" @input="recalcTemuanTotal(t)"
                                                   class="w-full rounded-xl border border-gray-300 px-3 py-2 text-xs font-semibold dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                                        </div>
                                        <div>
                                            <label class="mb-1 block text-[10px] font-bold uppercase text-gray-500">Kerugian Daerah (Rp)</label>
                                            <input type="number" :name="`temuans[${tIdx}][nilai_kerugian_daerah]`" x-model.number="t.nilai_kerugian_daerah" @input="recalcTemuanTotal(t)"
                                                   class="w-full rounded-xl border border-gray-300 px-3 py-2 text-xs font-semibold dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                                        </div>
                                        <div>
                                            <label class="mb-1 block text-[10px] font-bold uppercase text-gray-500">Kerugian Desa (Rp)</label>
                                            <input type="number" :name="`temuans[${tIdx}][nilai_kerugian_desa]`" x-model.number="t.nilai_kerugian_desa" @input="recalcTemuanTotal(t)"
                                                   class="w-full rounded-xl border border-gray-300 px-3 py-2 text-xs font-semibold dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                                        </div>
                                        <div>
                                            <label class="mb-1 block text-[10px] font-bold uppercase text-gray-500">Kerugian BOS/BLUD (Rp)</label>
                                            <input type="number" :name="`temuans[${tIdx}][nilai_kerugian_bos_blud]`" x-model.number="t.nilai_kerugian_bos_blud" @input="recalcTemuanTotal(t)"
                                                   class="w-full rounded-xl border border-gray-300 px-3 py-2 text-xs font-semibold dark:bg-gray-900 dark:border-gray-700 dark:text-white">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="md:col-span-2 rounded-xl bg-blue-50/70 p-3.5 border border-blue-100 dark:bg-blue-900/20 dark:border-blue-800 flex items-center justify-between">
                                    <span class="text-xs font-bold text-blue-900 dark:text-blue-300">Total Kerugian Temuan:</span>
                ...
                <div class="flex items-center justify-between border-t border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-900/30">
                    ...
                    <button type="button" @click="nextStep()"
                            class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-blue-700 active:scale-95 transition-all">
                        Lanjut ke Langkah 3: Edit Rekomendasi →
                    </button>
                </div>
            </div>
        </div>

        {{-- ── STEP 3: REKOMENDASI PER TEMUAN ── --}}
        <div x-show="step === 3" x-cloak>
            <div class="mb-6 rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-800">
                <div class="border-b border-gray-100 px-6 py-4 dark:border-gray-700 flex items-center justify-between">
                    <div>
                        <h2 class="text-sm font-bold text-gray-900 dark:text-white">Langkah 3: Edit Rekomendasi per Temuan</h2>
                        <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Tambahkan atau sesuaikan rekomendasi tindak lanjut untuk tiap temuan.</p>
                    </div>
                </div>
                    ...
                    <template x-for="(t, tIdx) in temuans" :key="t.id || t.key">
                        <div class="p-6 space-y-4">
                            ...
                            <div class="space-y-4 pl-3 border-l-2 border-blue-400 dark:border-blue-700">
                                <template x-for="(r, rIdx) in t.recommendations" :key="r.id || rIdx">
                                    <div class="rounded-xl border border-gray-200 bg-white p-4 space-y-4 dark:border-gray-700 dark:bg-gray-800/90 shadow-sm">
                                        <input type="hidden" :name="`temuans[${tIdx}][recommendations][${rIdx}][id]`" :value="r.id">
                ...
                <div class="flex items-center justify-between border-t border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-900/30">
                    ...
                    <button type="button" @click="nextStep()"
                            class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-blue-700 active:scale-95 transition-all">
                        Lanjut ke Langkah 4: Review &amp; Simpan →
                    </button>
                </div>
            </div>
        </div>
        ...
        <div x-show="step === 4" x-cloak>
            <div class="mb-6 rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-800">
                <div class="border-b border-gray-100 px-6 py-4 dark:border-gray-700">
                    <h2 class="text-sm font-bold text-gray-900 dark:text-white">Langkah 4: Review Perubahan &amp; Finalisasi</h2>
                </div>
                ...
                <div class="flex items-center justify-between border-t border-gray-100 bg-gray-50/50 px-6 py-4 dark:border-gray-800 dark:bg-gray-900/30">
                    ...
                    <button type="submit" id="btn-submit-edit"
                            class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-6 py-3 text-xs font-bold text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700 active:scale-95 transition-all">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                        </svg>
                        <span id="btn-submit-edit-text">Simpan Perubahan LHP</span>
                    </button>
                </div>
            </div>
        </div>
