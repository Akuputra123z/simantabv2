...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10 space-y-6">
    ...
    <form method="GET" action="{{ route('opd.tindak-lanjut.index') }}" class="rounded-xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-900/60">
        ...
        <div class="mt-3 flex items-center justify-between border-t border-slate-200/80 pt-3 dark:border-slate-800">
            ...
            <div class="flex items-center gap-2">
                @if($kategori !== 'PKPT' || $statusOpd || $status || $search)
                    <a href="{{ route('opd.tindak-lanjut.index') }}"
                       class="h-8 px-3 inline-flex items-center rounded-lg border border-slate-300 bg-white text-xs font-semibold text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 transition-colors">
                        Reset Filter
                    </a>
                @endif
                <button type="submit"
                        class="h-8 px-4 inline-flex items-center gap-1.5 rounded-lg bg-blue-700 text-xs font-semibold text-white hover:bg-blue-800 transition-colors shadow-xs">
