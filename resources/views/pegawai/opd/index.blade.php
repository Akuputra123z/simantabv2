...
<div class="p-6 bg-gray-50 min-h-screen dark:bg-gray-950">
    <div class="max-w-6xl mx-auto">
        ...
        <form method="GET" class="mb-4 flex flex-wrap gap-3">
            ...
            <select name="unit_opd" class="h-10 px-3 text-sm border rounded-lg">
                <option value="">Unit OPD</option>
            ...
            @if(request()->hasAny(['search', 'unit_opd']))
                <a href="{{ route('pegawai.opd.index') }}" class="h-10 px-4 text-sm border rounded-lg flex items-center">
                    Reset
                </a>
            @endif
        </form>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            <th class="px-5 py-3 text-xs font-bold text-gray-500 uppercase">Pegawai</th>
                            <th class="px-5 py-3 text-xs font-bold text-gray-500 uppercase">NIP</th>
                            <th class="px-5 py-3 text-xs font-bold text-gray-500 uppercase">Unit OPD</th>
                    ...
                    <tbody class="divide-y">
                        ...
                        <tr class="{{ ! $user->is_active ? 'opacity-60' : '' }}">
                            ...
                            <td class="px-5 py-4">
                                @if($user->opdUnits->isNotEmpty())
                                    @foreach($user->opdUnits as $unit)
                                        <span class="inline-block px-2 py-1 text-xs bg-blue-50 text-blue-700 rounded mb-1">
                                            {{ $unit->nama_unit }}
                                        </span>
                                    @endforeach
                                @else
                            ...
                            <td class="px-5 py-4">
                                <span class="px-2 py-1 text-xs rounded {{ $user->is_active ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }}">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <a href="{{ route('pegawai.opd.show', $user) }}"
                                       class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-400 transition hover:border-blue-200 hover:text-blue-600"
                                       title="Detail">
                                    ...
                                    <a href="{{ route('pegawai.opd.edit', $user) }}"
                                       class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-400 transition hover:border-gray-300 hover:text-gray-700"
                                       title="Edit">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    ...
                                    <form action="{{ route('pegawai.opd.destroy', $user) }}" method="POST"
                                        ...
                                        @csrf @method('DELETE')
                                        <button type="submit"
                                                class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-400 transition hover:border-red-200 hover:text-red-600"
                                                title="Hapus">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
